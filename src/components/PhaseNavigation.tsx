...
export const PhaseNavigation: React.FC<PhaseNavigationProps> = ({ steps, onStepClick }) => {
  return (
    <nav className="bg-[rgba(40,42,39,1)] shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1)] border flex w-full flex-col overflow-hidden items-stretch justify-center p-4 rounded-[20px] border-[rgba(112,112,112,1)] border-solid max-md:max-w-full">
      <div className="flex w-full items-center gap-[40px_83px] justify-between flex-wrap max-md:max-w-full">
        {steps.map((step) => (
          <button
            key={step.id}
            onClick={() => onStepClick?.(step.id)}
            className={`items-center self-stretch flex flex-col my-auto p-3.5 rounded-lg border-2 border-solid transition-colors ${
              step.isActive
          ...
          >
            ...
            <div className="text-white text-center text-xs font-medium leading-4">
              {step.title}
            </div>
          </button>
        ))}
